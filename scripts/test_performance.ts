...
async function main() {
  ...
  const trace1 = await fs.readFile(path.join(repoRoot, '.orchestrator', 'runs', runId1, 'trace.jsonl'), 'utf-8');
  const events1 = trace1.split('\n').filter(Boolean).map(line => JSON.parse(line));
  ...
  const trace2 = await fs.readFile(path.join(repoRoot, '.orchestrator', 'runs', runId2, 'trace.jsonl'), 'utf-8');
  const events2 = trace2.split('\n').filter(Boolean).map(line => JSON.parse(line));
  ...
  if (scanDuration2 >= scanDuration1) {
    throw new Error(`Scan with guardrails should be faster. No guardrails: ${scanDuration1}ms, with guardrails: ${scanDuration2}ms`);
  }
