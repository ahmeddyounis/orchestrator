...
L3 orchestration can be significantly more expensive than other modes. You can cap and tune it in `.orchestrator.yaml` (repo) or `~/.orchestrator/config.yaml` (user).

```yaml
# Note: bestOfN is capped by config validation (max 5).
l3:
  bestOfN: 3
  enableReviewer: true
  enableJudge: true
  diagnosis:
    enabled: true
    triggerOnRepeatedFailures: 2
    maxToTBranches: 3
```

- `l3.bestOfN`: Limits the maximum number of candidates (`--best-of N`) that can be generated in a single run.
- `l3.diagnosis.*`: Bounds the built-in diagnosis process when candidates fail verification.
...
```bash
orchestrator run --think L3 --best-of 3 "Fix the failing test in utils.test.ts"
```
...
After an L3 run, you can find the results in the run artifacts directory:

- `.orchestrator/runs/<run-id>/`

In addition to the standard artifacts (`summary.json`, `trace.jsonl`, `manifest.json`, `tool_logs/`, etc.), L3 produces extra artifacts that help you understand candidate generation and selection:

- `patches/iter_<n>_candidate_<i>.patch`: Candidate patches produced during iteration `n`.
- `patches/iter_<n>_selected.patch`: The selected candidate patch for iteration `n`.
- `patches/final.diff.patch`: The final unified diff for the full run.
- `verification/iter_<n>_candidate_<i>_report.json`: Verification reports for candidate patches.
- `selection/iter_<n>_ranking.json`: Candidate rankings for iteration `n`.

By inspecting these artifacts, you can understand why a particular solution was chosen and review alternative approaches that were considered.
