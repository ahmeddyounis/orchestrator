...
-   **If a candidate fails tests**, the **Judge** agent is invoked. The Judge performs a targeted, bounded diagnosis to understand the *reason* for the failure. It analyzes the test output and the candidate's code to determine if the failure is related to the proposed change or an unrelated issue.
