...
describe('confirm utility', () => {
  ...
  it('should return true when yes flag is set', async () => {
    const result = await confirm('Action', undefined, true, { yes: true, logger: mockLogger, runId });
  ...
  it('should return false when nonInteractive flag is set', async () => {
    const result = await confirm('Action', undefined, true, { nonInteractive: true, logger: mockLogger, runId });
  ...
  it('should prompt when interactive and return user selection (true)', async () => {
    ...
    expect(mockLogger.log).toHaveBeenCalledWith(expect.objectContaining({ type: 'ConfirmationRequested' }));
    expect(mockLogger.log).toHaveBeenCalledWith(expect.objectContaining({ 
        type: 'ConfirmationResolved', 
