...
class ProviderRegistry {
    ...
    getAdapter(providerId) {
        ...
        if (!providerConfig) {
            throw new shared_1.ConfigError(`Provider '${providerId}' not found in configuration.`);
        }
        // Look up factory
        const factory = this.factories.get(providerConfig.type);
        if (!factory) {
            throw new shared_1.ConfigError(`Unknown provider type '${providerConfig.type}' for provider '${providerId}'`);
        }
        // Validate env vars
        if (providerConfig.api_key_env && !providerConfig.api_key) {
            throw new shared_1.ConfigError(`Missing environment variable '${providerConfig.api_key_env}' for provider '${providerId}'`);
        }
        // Create adapter
