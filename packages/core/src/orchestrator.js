...
class Orchestrator {
  ...
  toArtifactRelPath(p) {
    if (!path_1.default.isAbsolute(p)) return p;
    const prefix = this.repoRoot.endsWith(path_1.default.sep)
      ? this.repoRoot
      : this.repoRoot + path_1.default.sep;
...
`;
    ...
    if (outputText) {
      await promises_1.default.writeFile(
        path_1.default.join(artifacts.root, 'executor_output.txt'),
        outputText,
      );
    }
    // 4. Parse Diff
  ...
  async runL1(goal, runId) {
    ...
    const executionService = new service_2.ExecutionService(
      eventBus,
      this.git,
      new repo_1.PatchApplier(),
      runId,
      this.repoRoot,
      this.config,
    );
    ...
    for (const step of steps) {
          ...
          try {
            ...
            if (fsSync.existsSync(path_1.default.join(indexPath, 'semantic.sqlite'))) {
              ...
              if (embedderId) {
                ...
                if (hits.length > 0) {
                  const hitsArtifactPath = path_1.default.join(
                    artifacts.root,
      ...
      const fusedJsonPath = path_1.default.join(
        artifacts.root,
        `fused_context_step_${stepsCompleted}_${stepSlug}.json`,
      );
      const fusedTxtPath = path_1.default.join(
        artifacts.root,
        `fused_context_step_${stepsCompleted}_${stepSlug}.txt`,
      );
      await promises_1.default.writeFile(
        fusedJsonPath,
        JSON.stringify(fusedContext.metadata, null, 2),
      );
...
PREVIOUS ATTEMPT FAILED. Error: ${lastError}\nPlease fix the error and try again.`;
  ...
  async runL2(goal, runId) {
    ...
    const verificationRunner = new runner_1.VerificationRunner(
      proceduralMemory,
      this.toolPolicy,
      this.ui,
      eventBus,
    ...
    const initialReportPath = path_1.default.join(
      artifacts.root,
...
`;
      ...
      if (outputText) {
        await promises_1.default.writeFile(
          path_1.default.join(artifacts.root, `repair_iter_${iterations}_output.txt`),
          outputText,
        );
      }
      const diffMatch = outputText?.match(/BEGIN_DIFF([\s\S]*?)END_DIFF/);
      ...
      const reportPath = path_1.default.join(
        artifacts.root,
        `verification_report_iter_${iterations}.json`,
      );
      await promises_1.default.writeFile(reportPath, JSON.stringify(verification, null, 2));
      reportPaths.push(reportPath);
      await promises_1.default.writeFile(
        path_1.default.join(artifacts.root, `verification_summary_iter_${iterations}.txt`),
        verification.summary,
      );
      if (verification.passed) {
        await eventBus.emit({
