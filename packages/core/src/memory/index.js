...
function ensureSqliteStore(dbPath) {
    if (sqliteStore && sqliteInitPath === dbPath)
        return sqliteStore;
    sqliteStore?.close();
...
class MemoryWriter {
    ...
    async extractEpisodic(runSummary, repoState, verificationReport, patchStats) {
        ...
        const { redacted: redactedContentPayload, redactionCount: contentRedactions } = (0, shared_1.redactUnknown)(contentPayload);
        ...
        const { redacted: redactedEvidence, redactionCount: evidenceRedactions } = (0, shared_1.redactUnknown)(evidence);
    ...
    async extractProcedural(toolRunMeta, toolRunResult, repoState) {
        ...
        if (exitCode !== 0 ||
            !applicableClassifications.includes(classification)) {
            return null;
        }
        const { redacted: normalizedCommand, redactionCount: commandRedactions } = (0, shared_1.redactString)(request.command.trim().replace(/\s+/g, ' '));
        await this.logRedactions(commandRedactions, 'procedural memory command');
        const existingMemory = [...memoryStore.values()].find((mem) => mem.type === 'procedural' && mem.content === normalizedCommand);
        const evidence = {
