import fs from 'fs';
import { CheckResult, FailedCheckSummary, FailureSummary } from './types';

export class FailureSummarizer {
  async summarize(checks: CheckResult[]): Promise<FailureSummary> {
    const failedChecks = checks.filter((c) => !c.passed);
    const summaries: FailedCheckSummary[] = [];
