...
describe('Orchestrator Deterministic E2E', () => {
  beforeAll(async () => {
    ...
    await fs.writeFile(path.join(TEST_ROOT, 'packages/a/src/target.ts'), 'export const value = 1;\n');
    await fs.writeFile(path.join(TEST_ROOT, 'packages/b/src/target.ts'), 'export const value = 1;\n');
