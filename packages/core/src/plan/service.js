...
class PlanService {
    ...
    async generatePlan(goal, providers, ctx, artifactsDir, repoRoot, config) {
        ...
        try {
            ...
            await fs.writeFile(path.join(artifactsDir, 'context_pack.json'), JSON.stringify(provenance, null, 2));
...
Each step should be a concise instruction.`;
        ...
        if (!response.text) {
            throw new shared_1.ProviderError('Planner provider returned empty response');
        }
        ...
        await fs.writeFile(path.join(artifactsDir, 'plan.json'), JSON.stringify({ steps: planSteps }, null, 2));
        await this.eventBus.emit({
            type: 'PlanCreated',
            schemaVersion: 1,
            timestamp: new Date().toISOString(),
