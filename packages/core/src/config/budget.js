...
function parseBudget(input) {
  ...
  for (const part of parts) {
    ...
    if (cleanKey === 'time') {
      ...
      if (isNaN(val)) throw new shared_1.ConfigError(`Invalid cost value: ${cleanVal}`);
      budget.cost = val;
    } else if (cleanKey === 'iter') {
      const val = parseInt(cleanVal, 10);
      if (isNaN(val)) throw new shared_1.ConfigError(`Invalid iter value: ${cleanVal}`);
      budget.iter = val;
    } else if (cleanKey === 'tool') {
      const val = parseInt(cleanVal, 10);
      if (isNaN(val)) throw new shared_1.ConfigError(`Invalid tool value: ${cleanVal}`);
      budget.tool = val;
    } else {
...
function parseDuration(input) {
  ...
  if (!match) {
    throw new shared_1.ConfigError(
