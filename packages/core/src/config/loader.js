...
class ConfigLoader {
  static loadYaml(filePath) {
    try {
      ...
      if (error instanceof js_yaml_1.default.YAMLException) {
        throw new shared_1.ConfigError(`Error parsing YAML file: ${filePath}\n${error.message}`);
      }
      throw error;
    }
  }
  ...
  static load(options = {}) {
    ...
    const userConfigPath = path_1.default.join(
    ...
    if (options.configPath) {
      if (!fs_1.default.existsSync(options.configPath)) {
        throw new shared_1.ConfigError(`Config file not found: ${options.configPath}`);
      }
    ...
    if (!result.success) {
      const issues = result.error.issues
        ...
        .join('\n');
      throw new shared_1.ConfigError(`Configuration validation failed:\n${issues}`);
    }
