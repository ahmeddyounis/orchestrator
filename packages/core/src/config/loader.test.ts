...
describe('ConfigLoader', () => {
  ...
  describe('load', () => {
    ...
    it('should load user config', () => {
      ...
      vi.mocked(fs.existsSync).mockImplementation((p) => p === path.join(mockHome, '.orchestrator', 'config.yaml'));
    ...
    it('should fail if explicit config file is missing', () => {
        expect(() => ConfigLoader.load({ configPath: '/missing.yaml' })).toThrow(/Config file not found/);
    });
    ...
    it('should fail on invalid YAML', () => {
        ...
        expect(() => ConfigLoader.load({ configPath: '/invalid.yaml' })).toThrow(/Error parsing YAML file/);
    });
    ...
    it('should fail on schema validation', () => {
        ...
        expect(() => ConfigLoader.load({ configPath: '/config.yaml' })).toThrow(/Configuration validation failed/);
    });
