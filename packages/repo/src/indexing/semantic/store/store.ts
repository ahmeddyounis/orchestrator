...
export class SemanticIndexStore {
  ...
  upsertEmbeddings(embeddings: Map<string, Float32Array>): void {
    const db = this.getDb()
    const transaction = db.transaction(() => {
      const stmt = db.prepare('INSERT OR REPLACE INTO semantic_embeddings (chunkId, vectorB64) VALUES (?, ?)')
  ...
  getAllEmbeddings(): Map<string, Float32Array> {
    const db = this.getDb();
    const stmt = db.prepare('SELECT chunkId, vectorB64 FROM semantic_embeddings WHERE vectorB64 IS NOT NULL');
