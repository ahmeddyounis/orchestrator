...
function createMemoryStore() {
    ...
    const upsert = (entry) => {
        if (!db)
            throw new shared_1.MemoryError('Database not initialized');
        const now = Date.now();
    ...
    const markVectorUpdated = (id) => {
        if (!db)
            throw new shared_1.MemoryError('Database not initialized');
        const now = Date.now();
        const stmt = db.prepare('INSERT OR REPLACE INTO memory_vectors (memory_id, updated_at) VALUES (?, ?)');
        stmt.run(id, now);
    };
    ...
    const status = (repoId) => {
        if (!db)
            throw new shared_1.MemoryError('Database not initialized');
        const countsStmt = db.prepare('SELECT type, COUNT(*) as count FROM memory_entries WHERE repoId = ? GROUP BY type');
        const countsRows = countsStmt.all(repoId);
        const lastUpdatedStmt = db.prepare('SELECT MAX(updatedAt) as lastUpdatedAt FROM memory_entries WHERE repoId = ?');
        const lastUpdatedRow = lastUpdatedStmt.get(repoId);
        const staleStmt = db.prepare('SELECT COUNT(*) as count FROM memory_entries WHERE repoId = ? AND stale = 1');
