...
describe("VectorBackendFactory", () => {
  describe("fromConfig", () => {
    ...
    it("should throw VectorBackendNotImplementedError for sqlite backend (not yet implemented)", () => {
      const config: VectorBackendConfig = { backend: "sqlite" };
      // sqlite is a local backend, so no remote opt-in error
      expect(() => VectorBackendFactory.fromConfig(config, false)).toThrow(
        VectorBackendNotImplementedError
      );
      expect(() => VectorBackendFactory.fromConfig(config, false)).toThrow(
        'Vector backend "sqlite" is not implemented.'
      );
    });
...
describe("MockVectorMemoryBackend", () => {
  ...
  describe("query", () => {
    ...
    it("should filter by type", async () => {
      ...
      expect(results.length).toBe(2);
      expect(results.every((r) => r.id === "item-1" || r.id === "item-3")).toBe(
        true
      );
    });
