...
class ProcessManager extends events_1.EventEmitter {
    ...
    async spawn(command, cwd, env, usePty) {
        if (this.ptyProcess || this.childProcess) {
            throw new shared_1.ProviderError('Process already running');
        }
        this.isPty = usePty;
    ...
    write(input) {
        ...
        if (this.isPty && this.ptyProcess) {
            ...
            throw new shared_1.ProviderError('Process not running');
        }
    }
    async readUntil(predicate, timeoutMs = 5000) {
        return new Promise((resolve, reject) => {
            ...
            const onExit = () => {
                cleanup();
                reject(new shared_1.ProviderError('Process exited while waiting'));
            };
            this.on('output', check);
            this.on('exit', onExit);
            timer = setTimeout(() => {
                cleanup();
                reject(new shared_1.ProviderError(`readUntil timed out after ${timeoutMs}ms`));
            }, timeoutMs);
        });
    }
    async readUntilHeuristic(silenceDurationMs, predicate, timeoutMs = 30000) {
        return new Promise((resolve, reject) => {
            ...
            totalTimeoutTimer = setTimeout(() => {
                cleanup();
                reject(new shared_1.ProviderError(`readUntilHeuristic timed out after ${timeoutMs}ms`));
            }, timeoutMs);
        });
    }
    ...
    handleOutput(chunk, stream) {
        const cleanChunk = (0, shared_1.stripAnsi)(chunk);
        if (this.options.maxOutputSize &&
            this.outputSize + cleanChunk.length > this.options.maxOutputSize) {
            this.kill('SIGKILL');
            this.emit('error', new shared_1.ProviderError(`Max output size ${this.options.maxOutputSize} exceeded`));
            return;
        }
        ...
        if (this.logger && this.pid) {
            const event = {
                ...
                payload: {
                    pid: this.pid,
                    stream,
                    chunk: (0, shared_1.redactForLogs)(cleanChunk),
                },
            };
            this.logger.log(event);
        }
