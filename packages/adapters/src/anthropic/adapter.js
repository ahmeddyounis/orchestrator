...
class AnthropicAdapter {
    ...
    constructor(config) {
        ...
        if (!apiKey) {
            throw new index_1.ConfigError(`Missing API Key for Anthropic provider. Checked config.api_key and env var ${config.api_key_env}`);
        }
    ...
    async *stream(req, ctx) {
        try {
            const stream = await (0, common_1.executeProviderRequest)(ctx, 'anthropic', this.model, async (signal) => {
                try {
                    const { system, messages } = this.mapMessages(req.messages);
                    const tools = this.mapTools(req.tools);
